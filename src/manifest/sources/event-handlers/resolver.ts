...
export function resolveEventHandler(params: {
  ...
  const { protocol, version, contractName, eventName, suffix } = params;
  ...
  try {
    ...
    return {
      ...
      handler: `handle${eventName}${suffix || ""}`,
    };
  } catch (error) {
...
function findEventInAbi(abiContent: AbiItem[], eventName: string, contractName: string): AbiItem {
  ...
  if (!event) {
    logger.error(`Event ${eventName} not found in ABI for ${contractName}`);
    process.exit(1);
  }

  if (!event.inputs) {
    logger.error(`Event ${eventName} has no inputs in ABI for ${contractName}`);
    process.exit(1);
  }
