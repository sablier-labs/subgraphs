...
export const definitions = {
  aliasNotFound: (release: Sablier.Release, chainId: number, contractName: string): never => {
    const chainName = getChainName(chainId);
    const msg = `Alias not found for contract ${contractName} in ${formatRelease(release)} on ${chainName}`;
    logAndThrow(msg);
  },
  blockNotFound: (release: Sablier.Release, chainId: number, contractName: string): never => {
    const chainName = getChainName(chainId);
    const msg = `Start block not found for contract ${contractName} in ${formatRelease(release)} on ${chainName}`;
    logAndThrow(msg);
  },
  contractsNotFound: (protocol: Indexed.Protocol, chainId: number): never => {
    const chainName = getChainName(chainId);
    const msg = `No contracts found for ${protocol} on ${chainName}`;
    logAndThrow(msg);
  },
  releaseNotFound: (protocol: Indexed.Protocol, version: Indexed.Version): never => {
