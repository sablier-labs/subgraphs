...
export async function Envio(endpoint: string, document: string, variables?: Record<string, unknown>) {
  return toEnvio(await request(endpoint, document, variables));
}

export async function TheGraph(endpoint: string, document: string, variables?: Record<string, unknown>) {
  return toTheGraph(await request(endpoint, document, variables));
}
