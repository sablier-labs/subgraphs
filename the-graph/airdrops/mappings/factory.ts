...
export function handleCreateCampaignLL_V21(event: EventCreateCampaignLL_V21): void {
  if (!isWhitelistedShape(event.params.lockupLinear)) {
    log.debug("Campaign skipped, shape contract not whitelisted: {}", [event.params.lockupLinear.toString()]);
    return;
  }
  ...
  if (action == null) {
    log.error("Campaign not registered yet, cannot bind action", []);
    return;
  }
...
export function handleCreateCampaignLL_V23(event: EventCreateCampaignLL_V23): void {
  if (!isWhitelistedShape(event.params.lockup)) {
    log.debug("Campaign skipped, shape contract not whitelisted: {}", [event.params.merkleLL.toString()]);
    return;
  }
...
export function handleCreateCampaignLT_V22(event: EventCreateCampaignLT_V22): void {
  if (!isWhitelistedShape(event.params.lockupTranched)) {
    log.debug("Campaign skipped, shape contract not whitelisted: {}", [event.params.lockupTranched.toString()]);
    return;
  }
...
export function handleCreateCampaignLT_V23(event: EventCreateCampaignLT_V23): void {
  if (!isWhitelistedShape(event.params.lockup)) {
    log.debug("Campaign skipped, shape contract not whitelisted: {}", [event.params.merkleLT.toString()]);
    return;
  }
