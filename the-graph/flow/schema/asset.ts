import { Address, BigInt as BInt, log } from "@graphprotocol/graph-ts";
import { ContractERC20, ContractERC20Bytes } from "../../erc20/bindings";
import { getChainId } from "../../shared";
import { DECIMALS_18, ONE, ZERO } from "../../shared/constants";
...
function getAssetName(address: Address): string {
  ...
  if (name.reverted) {
    ...
    if (nameBytes.reverted) {
      log.info("[SABLIER] Name reverted for token {}", [address.toHexString()]);
      return "Unknown";
    }
...
function getAssetSymbol(address: Address): string {
  ...
  if (symbol.reverted) {
    ...
    if (symbolBytes.reverted) {
      log.info("[SABLIER] Symbol reverted for token {}", [address.toHexString()]);
      return "Unknown";
    }
