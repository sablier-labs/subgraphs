import { BigInt as BInt, dataSource, log } from "@graphprotocol/graph-ts";
import { generateStreamAlias, generateStreamId, getChainId, getOrCreateBatch } from "../../shared";
...
export function createStream(event: EventCreate): EntityStream | null {
  ...
  if (contract == null) {
    log.info("[SABLIER] Contract not saved before this create event: {}", [contractId]);
    return null;
  }
