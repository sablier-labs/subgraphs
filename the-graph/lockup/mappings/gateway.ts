...
import { toEventAddress, toEventBoolean, toEventTuple, toValue } from "../../shared/convertors";
import {
  EventCancel_V20,
  EventCancel_V21_V22_V23,
  EventCreateDynamicMerged_V23,
  EventCreateDynamic_V20,
  EventCreateDynamic_V21,
  EventCreateDynamic_V22,
  EventCreateLinearMerged_V23,
  EventCreateLinear_V20,
  EventCreateLinear_V21,
  EventCreateLinear_V22,
  EventCreateTranchedMerged_V23,
  EventCreateTranched_V22,
  EventWithdraw_V20,
  EventWithdraw_V21_V22_V23,
} from "../bindings";
import {
  handleApproval,
...
function handleCreateLinear_V23(event_: EventCreateLinearMerged_V23): void {
  /** Handle amounts [deposit, protocol fee, broker fee] */
...
function handleCreateDynamic_V23(event_: EventCreateDynamicMerged_V23): void {
  /** Handle amounts [deposit, protocol fee, broker fee] */
  const deposit = toValue(event_.params.commonParams.amounts.deposit);
  const protocolFee = toValue(ZERO);
  const brokerFee = toValue(event_.params.commonParams.amounts.brokerFee);
...
function handleCreateTranched_V23(event_: EventCreateTranchedMerged_V23): void {
  /** Handle amounts [deposit, protocol fee, broker fee] */
  const deposit = toValue(event_.params.commonParams.amounts.deposit);
  const protocolFee = toValue(ZERO);
  const brokerFee = toValue(event_.params.commonParams.amounts.brokerFee);
