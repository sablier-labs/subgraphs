...
export function handleCancel(event: EventCancel): void {
  ...
  if (stream == null) {
    log.error("[SABLIER] Stream not saved before this cancel event: {}", [id.toHexString()]);
    return;
  }
...
export function handleTransfer(event: EventTransfer): void {
  ...
  const stream = getStreamByIdFromSource(id);
  if (stream == null) {
    log.error("[SABLIER] Stream not saved before this transfer event: {}", [id.toHexString()]);
    return;
  }
...
export function handleWithdraw(event: EventWithdraw): void {
  ...
  const stream = getStreamByIdFromSource(id);
  if (stream == null) {
    log.error("[SABLIER] Stream not saved before this withdraw event: {}", [id.toHexString()]);
    return;
  }
...
export function handleApproval(event: EventApproval): void {
  ...
  if (stream == null) {
    log.info("[SABLIER] Stream not saved before this approval event: {}", [id.toHexString()]);
    return;
  }
...
export function handleTransferAdmin(event: EventTransferAdmin): void {
  ...
  if (contract == null) {
    log.error("[SABLIER] Contract not saved before this transfer admin event: {}", [contractId]);
    return;
  }
