export const chainId = 1890;
export const chain = "mainnet";
export const startBlock_lockup = 63524900;
export const startBlock_merkle = 63526300;
export const startBlock_flow = 116397000;

/** Rule: keep addresses lowercased */

/**
 * Keep aliases unique and always in sync with the frontend
 * @example export let linear = [[address1, alias1, version1], [address2, alias2, version2]]
 */

export const linear: string[][] = [
  ["0x17c4f98c40e69a6A0D5c42B11E3733f076A99E20", "LL2", "V21"],
  ["0x6329591464fa6721c8e1c1271e4c6c41531aea6b", "LL3", "V22"],
];

export const dynamic: string[][] = [
  ["0x49d753422ff05daa291A9efa383E4f57daEAd889", "LD2", "V21"],
  ["0xaa05e418fb7851c211351c65435f1b17cbfa88bf", "LD3", "V22"],
];

export const tranched: string[][] = [["0x83403c6426e6d044bf3b84ec1c007db211aaa140", "LT3", "V22"]];

export const merged: string[][] = [["0x6c65aaf03186d1da60127d3d7792cf36ed99d909", "LK", "V23"]];

export const flow: string[][] = [
  ["0x46fa0164c5af9382d330e5a245a2ca8a18398950", "FL", "V10"],
  ["0x89d964e0b508234bcfdc7a32ae0aa0356f422b70", "FL2", "V11"],
];

export const factory: string[][] = [
  ["0xdB07a1749D5Ca49909C7C4159652Fbd527c735B8", "MSF2", "V21"],
  ["0x278ac15622846806bd46fbdbdb8db8d09614173a", "MSF3", "V22"],
  ["0xc0107f368fbb50075d2190549055d9e6bf75c5c9", "MSF4", "V23"],
];

/** PRBProxy registry */
export const registry = "";

/**
 * The initializer contract is used to trigger the indexing of all other contracts.
 */

export const initializer_lockup = linear[0][0];
export const initializer_merkle = factory[0][0];
export const initializer_flow = flow[0][0];
